...
.certificate {
    ...
    text-align: center;
    position: relative; /* ทำให้สามารถวางลำดับและปีการศึกษาที่มุมขวาบนได้ */
}
        ...
        @if ($registration->type === 'team')
            ...
            @if ($teamMembers && is_array($teamMembers))
                @foreach ($teamMembers as $member)
                    ...
                    <p>ได้<strong>{{ $registration->result }} {{ $registration->activity->name }}</strong></p>
                    ...
                    <div style="page-break-after: always;"></div> {{-- แบ่งหน้าแต่ละชื่อ --}}
                @endforeach
            @endif
        @else
